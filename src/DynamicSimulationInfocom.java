...
public class DynamicSimulationInfocom {
...
//			"nets/dgs/boids1000.dgs"
		...
		for (String net : networks)
			for (String algo : algorithms)
				for (long seed : seeds)
					new DynamicSimulationInfocom(net, algo, seed, args);

		return;

	}

	public DynamicSimulationInfocom(String network, String algorithm, long seed,
			String[] args) {
		System.out.println("----------");
...
//			NMI.init(graph);
			 ...
			 * Output file
			 */
			FileOutputStream out = new FileOutputStream("log_infocom/"
			...
			int step = 0;
			input.begin(network);
			while (input.nextStep() && step < 100000) {
				p.println("S = " + step);
